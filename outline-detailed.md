...
- 自己紹介は最小限。このトークのゴールを最初に宣言する:
  - JIT / OPcache がそれぞれ「どこに効くのか」を、**雰囲気ではなく計測で**理解する。
  - 「JIT 入れたのに速くならない」の正体を理解して帰ってもらう。
- PHP 実行の流れをざっくり図解:
  - `PHP ソース → 字句/構文解析 → OPCODE（中間表現）→ Zend VM が実行`
- **OPcache** とは:
  - コンパイル結果（OPCODE）をメモリにキャッシュし、毎リクエストの「ソース→OPCODE」変換を省く仕組み。
  - = **コンパイルコストの削減**であって、実行そのものを速くするわけではない。
- **JIT** とは（PHP 8.0+）:
 OPCODE の実行部分を速くする
...
## 2. OPcache が効くケース（まず軽く片付ける）

OPcache は話が単純なので **先に片付ける**。効果は **コード量（コンパイル対象の規模）にほぼ比例**する ―― 実行を速くするのではなく、毎リクエストの「ソース→OPCODE」変換を省くだけ。

- 薄い自作スクリプト … コンパイルコストが元々小さい → **効果も薄い**。
- 大量のクラス / ファイルを読む … 省略が積み上がる → **大きい**。

### ★ 一番効く例: Laravel（フレームワーク）

- Laravel は **1 リクエストで約 500 ファイル / 500 クラス**をロード・コンパイルする（実測: welcome ページで included **519** ファイル・declared **511** クラス）。これを毎回コンパイルし直すコストが支配的。
- OPcache ON なら、この「ソース→OPCODE」変換を **初回 1 回だけ**に圧縮。だから **フレームワークほど受益が大きい**。

#### 計測結果（実測 / Docker・PHP 8.5 / 1 リクエストのブート時間・30 回平均）

同一の Laravel リクエスト処理（約 500 ファイル）を、OPcache OFF と ON（コンパイル済み opcode を `file_cache` でプロセス跨ぎ再利用＝FPM 相当）で比較。

| 構成 | 1 リクエストのブート | 比 |
|------|---------------------|----|
| OPcache OFF | 92.70 ms | 1.0×（基準） |
| OPcache ON | **41.53 ms** | **約 2.2× 速い**（約 51ms 短縮） |

> **メッセージ**: OPcache は「実行」ではなく「**毎回のコンパイル**」を省く。コード量が多い **フレームワークが最大の受益者**で、薄いスクリプトでは効果は小さい。ここだけ押さえたら本題（JIT）へ。
...
- だから「ホットループ＋メソッド呼び出し」では **tracing が function を上回る**（§4-5 で実測）。§4-2 の「型付き値オブジェクトが効く」話も tracing 前提。
...
### 4-2. ★実践: 型付き値オブジェクト演算は JIT で大きく改善
...
### 4-3. ケース②: コールバックを取る標準関数（`array_map` / `usort`）
...
### 4-4. ケース③: 正規表現の「もう一つの JIT」= PCRE JIT
...
> 題材は **アロケーションを排した純粋比較**（§4-2 補足で予告したホットループ）。生成済みインスタンスのプロパティを、ホットループからメソッド経由で読んで加算するだけ。`new` / GC コストを取り除き、**JIT mode の差**と**型の差**だけを残す。
